...
class Churn2 extends Component {
	constructor(props) {
		super(props)
	...
	setupChurnData(props) {
		...
		let totalDataRR = CH.setStartingRRValues(churnTer)
		console.log(totalDataRR)
		...
		if(churnTer === "Global") {
			...
			detail = CH.totalGlobalClientsDetail(months, rawData, annual, project, statics, budget)
		} else {
			clients = CH.totalClients(months, rawData, annual, project, statics, budget, churnTer)
			detail = CH.totalClientsDetail(months, rawData, annual, project, statics, budget)
		}
		...
		let churnArray = CH.calculateChurn(churnDataArray, churnTer)
		...
		this.setState({
			...
			churnDataArray, currentPrevMonth, churnArray, churnTotalInAud, detail, addedTotalInAud, months
		})
